...
Every function wich accepts key expects `const char *k` pointer to data and it's size as `size_t`.

Some of the function (mainly range-query API) are not guaranteed to be implemented by all engines.
If an engine does not support ceratain function, it will return PMEMKV_STATUS_NOT_SUPPORTED.
...
:	Calls callback specified by `c` for every element stored in `db`. Arguments
...
:	Calls callback specified by `c` for every element stored in `db` which keys compare greater than
...
:	Calls callback specified by `c` for every element stored in `db` which keys compare less than
	specified key. Key is passed as pointer `const char *k` and size `kb`. Arguments
	passed to `c` are: pointer to key, size of the key, pointer to value, size of
	the value and `arg` specified by the user.
...
:	Calls callback specified by `c` on element with specified key. Key is passed as pointer `const char *k` and size `kb`.
	...
	Pointer to the value points to the location where data is actually stored (no copy occurs).
...
:	Returns human readible string describing last error.
