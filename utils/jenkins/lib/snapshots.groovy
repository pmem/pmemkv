...
/*
	...
	printf simulates user imput necessary to use Timeshift.
*/
	...
	sshagent (credentials: [ssh_credentials]) {
		libs.api.run_bash_script("""
			function wait_for_connection() {
				ssh -o ConnectTimeout=${con_timeout} -o ConnectionAttempts=${con_attempts} -o StrictHostKeyChecking=no -l ${user_name} ${ssh_address} 'true'
				echo \$?
			}
 ...
 * @param node_name node name as defined in Jenkins.
 * @return String Jenkins node IP address.
 */
