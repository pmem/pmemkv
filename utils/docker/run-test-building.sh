...
function build_with_flag() {
	CMAKE_FLAG_AND_SETTING=$1
	echo
	echo "##############################################################"
	echo "### Verifying building with flag: ${CMAKE_FLAG_AND_SETTING}"
	echo "##############################################################"
	mkdir $WORKDIR/build
	cd $WORKDIR/build

	cmake .. -D${CMAKE_FLAG_AND_SETTING}
	make -j$(nproc)
...
build_with_flag BUILD_JSON_CONFIG=OFF
