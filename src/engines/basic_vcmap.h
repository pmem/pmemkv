...
private:
	...
	using map_t = tbb::concurrent_hash_map<pmem_string, pmem_string,
					 tbb::tbb_hash_compare<pmem_string>,
					 std::scoped_allocator_adaptor<kv_allocator_t>>;

	/*  **config_** have to be declared before allocators and container. */
...
basic_vcmap<AllocatorFactory>::basic_vcmap(std::unique_ptr<internal::config> cfg)
    : config_(std::move(cfg)),
      kv_allocator(AllocatorFactory::template create<ch_allocator_t>(*config_)),
