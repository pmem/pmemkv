					       ...
					       const key_compare &comp)
{
	...
	iterator idxs_pos = lower_bound(entry.first, comp);
	if (idxs_pos != end() && !comp(idxs_pos->first, entry.first) &&
	    !comp(entry.first, idxs_pos->first)) {
		return std::pair<iterator, bool>(idxs_pos, false);
	}
	...
	pmem::obj::transaction::run(pop, [&] {
		...
		// update idxs
		entriy_pos = insert_idx(idxs_pos);
	});
					       ...
					       const key_compare &comp)
{
	...
	iterator idxs_pos = lower_bound(std::forward<K>(key), comp);
	if (idxs_pos != end() && !comp(idxs_pos->first, std::forward<K>(key)) &&
	    !comp(std::forward<K>(key), idxs_pos->first)) {
		return std::pair<iterator, bool>(idxs_pos, false);
	}
...
template <typename Key, typename T, typename Compare, std::size_t degree>
constexpr typename b_tree_base<Key, T, Compare, degree>::size_type
b_tree_base<Key, T, Compare, degree>::max_size() const noexcept
{
