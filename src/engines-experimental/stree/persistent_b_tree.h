...
private:
	...
	void add_to_tx(size_type idx, size_type count);
}; /* class leaf_node_t */
						  ...
						  const key_compare &comp)
{
	...
	pmem::obj::transaction::run(pop, [&] {
		...
		other->add_to_tx(middle_idx, other->size() - middle_idx);
...
/**
 * Adds the range [idx, idx + count) of underlying array to transaction.
 */
template <typename Key, typename T, typename Compare, uint64_t capacity>
void leaf_node_t<Key, T, Compare, capacity>::add_to_tx(size_type idx, size_type count)
{
	assert(pmemobj_tx_stage() == TX_STAGE_WORK);
	pmemobj_tx_add_range_direct(entries + idx, sizeof(value_type) * count);
}
...
{
	if (key.data() == std::string("10687")) {
		std::cout << "ERASE KEY: " << key.data() << std::endl;
	}
	assert(root != nullptr);
