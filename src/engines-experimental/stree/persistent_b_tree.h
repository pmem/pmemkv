...
private:
	...
	size_type insert_idx(difference_type new_entry_idx, const_iterator hint);
...
{
	assert(pmemobj_tx_stage() == TX_STAGE_WORK);
	assert(idxs[size()] == new_entry_idx);

	auto idx_hint = static_cast<size_type>(std::distance(cbegin(), hint));
	auto slice = idxs.range(idx_hint, size() - idx_hint);
...
void inner_node_t<Key, Compare, capacity>::move_first(persistent_ptr<inner_node_t> other)
{
	...
	std::move(other->entries + 1, other->entries + other->size(), other->entries);
	std::move(other->children + 1, other->children + other->size() + 1,
		  other->children);
...
void inner_node_t<Key, Compare, capacity>::move_last(persistent_ptr<inner_node_t> other)
{
	...
	entries[0] = std::move(other->entries[other->size() - 1]);
	children[0] = std::move(other->children[other->size()]);
