...
private:
	...
	bool is_left_node(const leaf_type *src_node, const leaf_type *lnode);
	bool is_right_node(const leaf_type *src_node, const leaf_type *rnode);
	...
	void correct_leaf_node_links(pool_base &, node_pptr &, node_pptr &, node_pptr &);

	leaf_type *find_leaf_node(const key_type &key) const;
	template <typename K>
	leaf_type *find_leaf_node(K &&key) const;
	// TODO: merge with previous method
	leaf_pptr find_leaf_to_insert(const key_type &key, path_type &path) const;
	template <typename K>
	...
	template <typename... Args>
	inline inner_pptr allocate_inner(pool_base &pop, Args &&... args);
	template <typename... Args>
	inline leaf_pptr allocate_leaf(pool_base &pop, Args &&... args);
	inline void deallocate(node_pptr &node);
	inline void deallocate_inner(inner_pptr &node);
	inline void deallocate_leaf(leaf_pptr &node);
...
{
	auto it = std::upper_bound(begin(), end(), std::forward<K>(key));
...
{
	assert(size() > 0);
	iterator it = std::upper_bound(begin(), end(), std::forward<K>(key));
	if (it == begin()) {
...
b_tree_base<Key, T, Compare, degree>::try_emplace(K &&key, M &&obj)
{
	...
	if (root == nullptr) {
		pmem::obj::transaction::run(pop, [&] {
			root = pmem::obj::make_persistent<leaf_type>(epoch);
		});
	}
	...
	node_pptr node = root;
	while (!node->leaf()) {
		path.push_back(cast_inner(node).get());
	...
	// -------------------- if root is leaf ------------------------
	if (path.empty()) {
		iterator it = split_leaf_node(pop, cast_leaf(node), std::forward<K>(key),
					      std::forward<K>(obj));
...
size_t b_tree_base<Key, T, Compare, degree>::erase(K &&key)
{
	...
	auto get_suitable_leaf = [this](inner_pair &pair) -> leaf_pptr {
	...
	pmem::obj::transaction::run(pop, [&] {
		...
		if (path.empty()) {
			pmem::obj::delete_persistent<leaf_type>(leaf);
			return;
		}
		/* handle leaf node */
		...
		while (path.size() > 1 && node->size() == 0) {
			...
			pmem::obj::delete_persistent<inner_type>(node);
							   ...
							   node_pptr &r_child)
{
	...
	assert(pmemobj_tx_stage() == TX_STAGE_WORK);
	cast_inner(root) = allocate_inner(pop, root->level() + 1, key, l_child, r_child);
}
						 ...
						 key_pptr &partition_key)
{
	...
	other = allocate_inner(pop, node->level());
						      ...
						      M &&obj)
{
	...
	pmem::obj::transaction::run(pop, [&] {
		...
		cast_inner(root) = allocate_inner(pop, root->level() + 1,
						  node->front().first, split_leaf, node);
...
void b_tree_base<Key, T, Compare, degree>::repair_leaf_split(pool_base &pop)
{
	...
	leaf_pptr found_node = find_leaf_to_insert(key, path);
	assert(path[0] == root);
	...
	if (split_node == found_node) { // Split not completed
		...
		if (left_child && is_left_node(split_leaf, lnode)) {
			if (right_child &&
				...
				if (parent_node) {
					parent_node->update_splitted_child(
						pop, rnode->front().first, left_child,
						right_child);
				} else {
...
template <typename Key, typename T, typename Compare, size_t degree>
void b_tree_base<Key, T, Compare, degree>::correct_leaf_node_links(pool_base &pop,
								   node_pptr &src_node,
								   node_pptr &left,
								   node_pptr &right)
{
	leaf_pptr lnode = cast_leaf(left);
	leaf_pptr rnode = cast_leaf(right);
...
template <typename Key, typename T, typename Compare, size_t degree>
inline void b_tree_base<Key, T, Compare, degree>::deallocate_inner(inner_pptr &node)
{
	delete_persistent<inner_type>(node);
}

template <typename Key, typename T, typename Compare, size_t degree>
inline void b_tree_base<Key, T, Compare, degree>::deallocate_leaf(leaf_pptr &node)
{
