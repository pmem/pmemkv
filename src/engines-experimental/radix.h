...
struct pmem_type {
	...
	map_type_generic<Mt> map;
	pmem::obj::persistent_ptr<log_type> log;
	uint64_t reserved[6];
};

static_assert(sizeof(pmem_type<false>) == sizeof(map_type) + 64, "");
 ...
 */
class radix : public pmemobj_engine_base<internal::radix::pmem_type<false>> {
...
private:
	using pmem_type = internal::radix::pmem_type<false>;
 ...
 */
class heterogeneous_radix : public pmemobj_engine_base<internal::radix::pmem_type<true>> {
public:
...
private:
	using pmem_type = internal::radix::pmem_type<true>;
	...
	struct value_accessor {
		value_accessor();
		value_accessor(unique_ptr_type data_, size_t size_);

		size_t size() const;
		const char *data() const;

	private:
		unique_ptr_type data_;
		size_t size_;
	};
	...
	struct merged_iterator {
		...
		heterogeneous_radix::value_accessor value() const;
