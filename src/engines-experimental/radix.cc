...
string_view heterogeneous_radix::merged_iterator::value() const
{
	assert(dereferenceable());
    ...
    : pmemobj_engine_base(cfg, "pmemkv_radix"), config(std::move(cfg))
{
	...
	if (!cfg->get_uint64("log_size", &log_size))
		throw internal::invalid_argument("Log size not specified.");
	if (!cfg->get_uint64("cache_size", &cache_size))
		throw internal::invalid_argument("Cache size not specified.");
...
bool heterogeneous_radix::log_contains(const void *ptr)
{
					      ...
					      bool update_dram)
{
	 ...
	 *
	 * If update_dram is set it also exchanges pointer in dram_entry so
	...
	const uvalue_type *expected;
	const uvalue_type *desired;

	if (e->remove) {
		container->erase(e->key());

		expected = tombstone_volatile();
