...
static status iterate(Iterator first, get_kv_callback *callback, void *arg, Predicate &&p)
{
...
heterogeneous_radix::value_accessor::value_accessor() : data_(nullptr, &no_delete)
{
}

heterogeneous_radix::value_accessor::value_accessor(unique_ptr_type data_, size_t size_)
    : data_(std::move(data_)), size_(size_)
{
}

size_t heterogeneous_radix::value_accessor::size() const
{
	return size_;
}

const char *heterogeneous_radix::value_accessor::data() const
{
	return data_.get();
}
...
heterogeneous_radix::value_accessor heterogeneous_radix::merged_iterator::value() const
{
	assert(dereferenceable());
	value_accessor val_acc;

	hetero_radix.container_worker->critical([&] {
		if (curr_it == current_it::dram) {
			auto v = &dram_it->second->second;
			auto value = v->load(std::memory_order_acquire);

			assert(value != tombstone_persistent() &&
			...
			val_acc = value_accessor(std::move(ptr), size);
		} else {
			auto val = pmem_it->value();

			unique_ptr_type ptr(val.data(), no_delete);
			val_acc = value_accessor(std::move(ptr), val.size());
		}
	});
...
heterogeneous_radix::heterogeneous_radix(std::unique_ptr<internal::config> cfg)
    : pmemobj_engine_base(cfg, "pmemkv_radix"), config(std::move(cfg))
{
	cfg->get_uint64("log_size", &log_size);
	cfg->get_uint64("cache_size", &cache_size);
...
status heterogeneous_radix::remove(string_view k)
{
	...
	/* Check if element exists. It is needed to return proper
...
status heterogeneous_radix::get(string_view key, get_v_callback *callback, void *arg)
{
	...
	container_worker->critical([&] {
		...
		while (true) {
			...
			auto size = value->size();
			auto ptr = try_read_value(v, value);
			if (!ptr)
				continue;
