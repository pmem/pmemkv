...
static void hashmap_create(PMEMobjpool *pop, TOID(struct hashmap_rp) * hashmap_p)
{
	...
	if (TOID_IS_NULL(hashmap)) {
		// LOG("hashmap alloc failed: " + pmemobj_errormsg());
	...
	D_RW(hashmap)->resize_threshold =
		static_cast<uint64_t>(INIT_ENTRIES_NUM_RP * HASHMAP_RP_LOAD_FACTOR);
			 ...
			 size_t capacity_new)
{
	...
	uint64_t resize_threshold_new =
		static_cast<uint64_t>(capacity_new * HASHMAP_RP_LOAD_FACTOR);
...
static size_t shard_hash(uint64_t key)
{
	...
	return static_cast<size_t>(mix(h) & (SHARDS - 1));
}
