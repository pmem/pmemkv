...
class pmemobj_less_compare {
public:
	using is_transparent = void;

	pmemobj_less_compare() : name()
	{
	}

	void initialize(const comparator *cmp)
	{
		/* cmp is restored on every restart */
		VALGRIND_PMC_REMOVE_PMEM_MAPPING(&this->cmp, sizeof(this->cmp));
...
static_assert(sizeof(pmemobj_less_compare) == 40, "wrong persistent_less_compare size");
