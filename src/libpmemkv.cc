...
{
	try {
		pmem::kv::internal::iterator<true> *it = nullptr;
		db_to_internal(db)->new_const_iterator(it);
		return iterator_from_internal(it);
	} catch (const std::exception &exc) {
		ERR() << exc.what();
		return nullptr;
	} catch (...) {
...
{
	try {
		pmem::kv::internal::iterator<false> *it = nullptr;
		db_to_internal(db)->new_iterator(it);
		return iterator_from_internal(it);
	} catch (const std::exception &exc) {
		ERR() << exc.what();
		return nullptr;
...
/* void * ? */
void pmemkv_read_iterator_delete(pmemkv_read_iterator *it)
{
	try {
		delete iterator_to_internal(it);
	} catch (const std::exception &exc) {
		ERR() << exc.what();
	} catch (...) {
		ERR() << "Unspecified failure";
