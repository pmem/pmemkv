...
private:
	...
	std::unique_ptr<pmemkv_config, std::function<void(pmemkv_config *)>> _config;
};
...
private:
	std::unique_ptr<pmemkv_db, std::function<void(pmemkv_db *)>> _db;
};
...
inline config::config() noexcept
    : _config(nullptr, [](pmemkv_config *c) { pmemkv_config_delete(c); })
{
}
...
inline config::config(pmemkv_config *cfg) noexcept
    : _config(cfg, [](pmemkv_config *c) { pmemkv_config_delete(c); })
{
}
...
{
	if (this->_config.get() == nullptr) {
		this->_config = {pmemkv_config_new(),
				 [](pmemkv_config *c) { pmemkv_config_delete(c); }};
			       ...
			       const std::size_t count) noexcept
{
	...
	return static_cast<status>(pmemkv_config_put_data(
		this->_config.get(), key.data(), (void *)value, count * sizeof(T)));
}
				 ...
				 void (*deleter)(void *)) noexcept
{
	...
	return static_cast<status>(pmemkv_config_put_object(
		this->_config.get(), key.data(), (void *)value, deleter));
}
				 ...
				 std::unique_ptr<T, D> object) noexcept
{
	...
	return static_cast<status>(pmemkv_config_put_object_cb(
		this->_config.get(), key.data(), (void *)wrapper, internal::call_up_get,
...
inline status config::put_comparator(Comparator &&comparator)
{
	...
	return static_cast<status>(pmemkv_config_put_object_cb(
		this->_config.get(), "comparator", (void *)entry, internal::call_up_get,
		internal::call_up_destructor));
}
...
inline status config::put_uint64(const std::string &key, std::uint64_t value) noexcept
{
	...
	return static_cast<status>(
		pmemkv_config_put_uint64(this->_config.get(), key.data(), value));
}
...
inline status config::put_int64(const std::string &key, std::int64_t value) noexcept
{
	...
	return static_cast<status>(
		pmemkv_config_put_int64(this->_config.get(), key.data(), value));
}
				 ...
				 const std::string &value) noexcept
{
	...
	return static_cast<status>(
		pmemkv_config_put_string(this->_config.get(), key.data(), value.data()));
}
...
inline status config::put_oid(PMEMoid *oid) noexcept
{
	...
	return static_cast<status>(pmemkv_config_put_oid(this->_config.get(), oid));
}
			       ...
			       std::size_t &count) const noexcept
{
	...
	auto s = static_cast<status>(pmemkv_config_get_data(
		this->_config.get(), key.data(), (const void **)&value, &size));
...
inline status config::get_object(const std::string &key, T *&value) const noexcept
{
	...
	auto s = static_cast<status>(pmemkv_config_get_object(
		this->_config.get(), key.data(), (void **)&value));
	...
	noexcept
{
	...
	return static_cast<status>(
		pmemkv_config_get_uint64(this->_config.get(), key.data(), &value));
}
	...
	noexcept
{
	...
	return static_cast<status>(
		pmemkv_config_get_int64(this->_config.get(), key.data(), &value));
}
	...
	noexcept
{
	...
	auto s = static_cast<status>(
		pmemkv_config_get_string(this->_config.get(), key.data(), &data));
 ...
 */
inline db::db() noexcept : _db(nullptr, [](pmemkv_db *d) { pmemkv_close(d); })
{
}
...
{
	if (this->_db.get() != nullptr)
		pmemkv_close(this->_db.get());
...
{
	return static_cast<status>(pmemkv_count_all(this->_db.get(), &cnt));
}
...
{
	return static_cast<status>(
		pmemkv_count_above(this->_db.get(), key.data(), key.size(), &cnt));
}
...
{
	return static_cast<status>(
		pmemkv_count_equal_above(this->_db.get(), key.data(), key.size(), &cnt));
}
...
{
	return static_cast<status>(
		pmemkv_count_equal_below(this->_db.get(), key.data(), key.size(), &cnt));
}
...
{
	return static_cast<status>(
		pmemkv_count_below(this->_db.get(), key.data(), key.size(), &cnt));
}
...
inline status db::count_between(string_view key1, string_view key2,
				std::size_t &cnt) noexcept
{
	return static_cast<status>(pmemkv_count_between(this->_db.get(), key1.data(),
							key1.size(), key2.data(),
							key2.size(), &cnt));
}
...
{
	return static_cast<status>(pmemkv_get_all(this->_db.get(), callback, arg));
}
...
{
	return static_cast<status>(
		pmemkv_get_all(this->_db.get(), call_get_kv_function, &f));
}
...
{
	return static_cast<status>(
		pmemkv_get_above(this->_db.get(), key.data(), key.size(), callback, arg));
}
...
inline status db::get_above(string_view key, std::function<get_kv_function> f) noexcept
{
	return static_cast<status>(pmemkv_get_above(
		this->_db.get(), key.data(), key.size(), call_get_kv_function, &f));
}
...
inline status db::get_equal_above(string_view key, get_kv_callback *callback,
				  void *arg) noexcept
{
	return static_cast<status>(pmemkv_get_equal_above(this->_db.get(), key.data(),
...
inline status db::get_equal_below(string_view key, get_kv_callback *callback,
				  void *arg) noexcept
{
	return static_cast<status>(pmemkv_get_equal_below(this->_db.get(), key.data(),
							  key.size(), callback, arg));
}
...
{
	return static_cast<status>(
		pmemkv_get_below(this->_db.get(), key.data(), key.size(), callback, arg));
}
...
{
	return static_cast<status>(pmemkv_get_between(this->_db.get(), key1.data(),
						      key1.size(), key2.data(),
...
{
	return static_cast<status>(
		pmemkv_get_between(this->_db.get(), key1.data(), key1.size(), key2.data(),
...
{
	return static_cast<status>(
		pmemkv_exists(this->_db.get(), key.data(), key.size()));
}
...
{
	return static_cast<status>(
		pmemkv_get(this->_db.get(), key.data(), key.size(), callback, arg));
}
...
{
	return static_cast<status>(pmemkv_get(this->_db.get(), key.data(), key.size(),
...
{
	return static_cast<status>(pmemkv_put(this->_db.get(), key.data(), key.size(),
...
{
	return static_cast<status>(
		pmemkv_remove(this->_db.get(), key.data(), key.size()));
}
...
{
	return static_cast<status>(
		pmemkv_defrag(this->_db.get(), start_percent, amount_percent));
}
