 ...
 * If true: pmemkv creates the file, unless it exists - then it fails.
 * If false: pmemkv opens the file, unless the path does not exist - then it fails.
 * False by default.
 *
 * @return pmem::kv::status
 */
 ...
 * Works only with engines supporting this flag and it means:
 * If true: pmemkv tries to open the file and if that doesn't succeed
 *	  it means there's (most likely) no file to use, so it tries to create it.
 * If false: pmemkv creates or opens the file based on setting of
