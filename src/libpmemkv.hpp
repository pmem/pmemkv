...
/*! \class db::iterator
	...
	Both iterators types (write_iterator and read_iterator) allows reading record's
...
/*! \class db::iterator::OutputIterator
	\brief OutputIterator provides iteration through elements without an option of
...
/**
 * Returns record's key (string_view), in pmem::kv::result<string_view>.
 *
 * If the iterator is on an undefined position, calling this method is undefined
 * behaviour.
 *
 * @return pmem::kv::result<string_view>
 */
template <bool IsConst>
...
/**
 * Returns value's range (string_view) to read, in pmem::kv::result.
 *
 * It is only used to read a value, if you want to modify the value, use write_range.
 *
 * If the iterator is on an undefined position, calling this method is undefined
 * behaviour.
 *
 * @param[in] pos position of value's element which will be the first element in the range
 * (defaultly = 0)
 * @param[in] n number of elements in range (defaultly =
 * std::numeric_limits<size_t>::max(), if n is bigger than number of elements in a value
 * it's automatically shrinked)
 *
 * @return pmem::kv::result<string_view>
 */
template <bool IsConst>
...
/**
 * Returns value's range (pmem::obj::slice<pmem::kv::OutputIterator<char *>>) to modify,
 * in pmem::kv::result.
 *
 * It is only used to modify a value, if you want to read the value, use read_range.
 *
