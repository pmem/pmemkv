...
static void seek_lower_test(pmem::kv::db &kv)
{
	...
	std::for_each(keys.begin() + 1, keys.end(), [&](pair p) {
		...
		verify_value<IsConst>(it.value(), (prev_key++)->second);
	});
}
...
static void seek_higher_test(pmem::kv::db &kv)
{
	...
	std::for_each(keys.begin(), keys.end() - 1, [&](pair p) {
		...
		verify_value<IsConst>(it.value(), (next_key++)->second);
	});
...
static void next_test(pmem::kv::db &kv)
{
	...
	verify_value<IsConst>(it.value(), (--keys.end())->second);
	ASSERT_STATUS(it.next(), pmem::kv::status::NOT_FOUND);
}
...
static void prev_test(pmem::kv::db &kv)
{
	...
	verify_value<IsConst>(it.value(), keys.begin()->second);
	ASSERT_STATUS(it.prev(), pmem::kv::status::NOT_FOUND);
}
...
static void test(int argc, char *argv[])
{
	...
	run_engine_tests(argv[1], argv[2],
			 {seek_lower_test<true>, seek_lower_test<false>,
			  seek_lower_eq_test<true>, seek_lower_eq_test<false>,
			  seek_higher_test<true>, seek_higher_test<false>,
			  seek_higher_eq_test<true>, seek_higher_eq_test<false>,
			  next_test<true>, next_test<false>});
