...
static void CommonGetTest(std::string engine, pmem::kv::config &&config)
{
	...
	verify_get_all(kv, 6, expected);
	verify_get_above(kv, EMPTY_KEY, 6, expected);
	...
	expected = kv_list{{"A", "1"},	{"AB", "2"}, {"AC", "3"},  {"B", "4"},
			   {"BB", "5"}, {"BC", "6"}, {"记!", "RR"}};
	verify_get_between(kv, EMPTY_KEY, MAX_KEY, expected.size(), expected);
	verify_get_equal_above(kv, EMPTY_KEY, expected.size(), expected);
	verify_get_above(kv, EMPTY_KEY, expected.size(), expected);
	verify_get_below(kv, MAX_KEY, expected.size(), expected);
	verify_get_equal_below(kv, MAX_KEY, expected.size(), expected);
	verify_get_all(kv, expected.size(), expected);
	...
	expected = kv_list{{"A", "1"}, {"AB", "2"}, {"AC", "3"},
			   {"B", "4"}, {"BB", "5"}, {"BC", "6"}};
	verify_get_below(kv, "Z", 6, expected);
	verify_get_between(kv, EMPTY_KEY, "Z", 6, expected);
	verify_get_equal_below(kv, "Z", 6, expected);
	verify_get_above(kv, EMPTY_KEY, 6, expected);
	verify_get_all(kv, 6, expected);
	verify_get_equal_above(kv, EMPTY_KEY, 6, expected);
