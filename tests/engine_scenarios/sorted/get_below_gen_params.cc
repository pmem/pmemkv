...
static void GetBelowTest(std::string engine, pmem::kv::config &&config)
{
	...
	auto expected = kv_list{{"A", "1"}, {"AB", "2"}, {"AC", "3"},
				{"B", "4"}, {"BB", "5"}, {"BC", "6"}};
	verify_get_below(kv, MAX_KEY, 6, expected);

	expected = kv_list{{"A", "1"}, {"AB", "2"}, {"AC", "3"}};
	verify_get_below(kv, "B", 3, expected);
	...
	expected = kv_list{{"A", "1"},	{"AB", "2"}, {"AC", "3"}, {"B", "4"},
			   {"BB", "5"}, {"BC", "6"}, {"BD", "7"}};
	verify_get_below(kv, "ZZZ", 7, expected);

	expected = kv_list{{"A", "1"}, {"AB", "2"}, {"AC", "3"},
			   {"B", "4"}, {"BB", "5"}, {"BC", "6"}};
	verify_get_below(kv, "BD", 6, expected);
	...
	verify_get_below(kv, MAX_KEY, 8, expected);

	/* testing C-like API */
	expected = kv_list{{"A", "1"},	{"AB", "2"}, {"AC", "3"}, {"B", "4"},
			   {"BB", "5"}, {"BC", "6"}, {"BD", "7"}};
	verify_get_below_c(kv, "BE", 7, expected);
	verify_get_below_c(kv, "记!", 7, expected);
...
static void GetBelowTest2(std::string engine, pmem::kv::config &&config)
{
	...
	expected = kv_list{{"aaa", "1"}, {"bbb", "2"}, {"ccc", "3"},
			   {"rrr", "4"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_below_c(kv, MAX_KEY, 6, expected);

	expected = kv_list{{"aaa", "1"}, {"bbb", "2"}, {"ccc", "3"},
			   {"rrr", "4"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_below_c(kv, "z", 6, expected);
			     ...
			     const size_t items, const size_t max_key_len)
{
	...
	for (size_t i = 0; i < items; i++) {
		...
		verify_get_below(kv, MAX_KEY, i + 1, expected);
			     ...
			     const size_t max_key_len)
{
	...
	verify_get_below(kv, MAX_KEY, 2, expected);
	...
	for (size_t i = 0; i < keys_cnt; i++) {
		...
		if (i % 5 == 0) {
			/* verifies all elements */
			verify_get_below(kv, MAX_KEY, i + 3, expected);
				    ...
				    const size_t max_key_len)
{
	...
	for (size_t i = keys_cnt; i > 0; i--) {
		...
		/* verifies all elements */
		verify_get_below(kv, MAX_KEY, curr_iter + 1, expected);
	...
	if (keys_cnt > 20) {
		...
		/* verifies all elements */
		verify_get_below_c(kv, MAX_KEY, keys_cnt, expected);
	}
	if (keys_cnt > 9) {
		key = keys[8];
		UT_ASSERTeq(kv.get(key, &value), status::OK);
