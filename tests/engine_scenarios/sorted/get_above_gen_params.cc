...
static void GetAboveTest(std::string engine, pmem::kv::config &&config)
{
	...
	verify_get_above(kv, "B", 2, expected);
	...
	expected = kv_list{{"A", "1"},	{"AB", "2"}, {"AC", "3"}, {"B", "4"},
			   {"BB", "5"}, {"BC", "6"}, {"BD", "7"}};
	verify_get_above(kv, EMPTY_KEY, 7, expected);

	verify_get_above(kv, "ZZZ", 0, kv_list());

	expected = kv_list{{"BB", "5"}, {"BC", "6"}, {"BD", "7"}};
	verify_get_above(kv, "BA", 3, expected);
	...
	verify_get_above_c(kv, "B", 4, expected);
...
static void GetAboveTest2(std::string engine, pmem::kv::config &&config)
{
	...
	auto expected = kv_list{{"aaa", "1"}, {"bbb", "2"}, {"ccc", "3"},  {"rrr", "4"},
				{"sss", "5"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_above_c(kv, MIN_KEY, 7, expected);

	expected = kv_list{{"rrr", "4"}, {"sss", "5"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_above_c(kv, "ccc", 4, expected);

	expected = kv_list{{"aaa", "1"}, {"bbb", "2"}, {"ccc", "3"},  {"rrr", "4"},
			   {"sss", "5"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_above_c(kv, "a", 7, expected);

	expected = kv_list{{"rrr", "4"}, {"sss", "5"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_above_c(kv, "ddd", 4, expected);

	/* remove one key */
	UT_ASSERTeq(kv.remove("sss"), status::OK);

	expected = kv_list{{"rrr", "4"}, {"ttt", "6"}, {"yyy", "记!"}};
	verify_get_above_c(kv, "ddd", 3, expected);
			     ...
			     const size_t items, const size_t max_key_len)
{
	...
	for (size_t i = 0; i < items; i++) {
		...
		verify_get_above(kv, MIN_KEY, i + 1, expected);
			     ...
			     const size_t max_key_len)
{
	...
	verify_get_above(kv, MIN_KEY, 2, expected);
	...
	for (size_t i = 0; i < keys_cnt; i++) {
		...
		if (i % 5 == 0) {
			/* verifies all elements */
			verify_get_above(kv, MIN_KEY, i + 3, expected);

			/* verifies almost all elements */
			auto exp_sorted = kv_sort(expected);
			verify_get_above(
				kv, exp_sorted[0].first, i + 2,
				kv_list(exp_sorted.begin() + 1, exp_sorted.end()));
		}
	}
				    ...
				    const size_t max_key_len)
{
	...
	for (size_t i = keys_cnt; i > 0; i--) {
		...
		/* verifies all elements */
		verify_get_above(kv, MIN_KEY, keys_cnt - i + 1, expected);
	...
	if (keys_cnt > 20) {
		...
		/* verifies all elements */
		verify_get_above_c(kv, MIN_KEY, keys_cnt, expected);
	}
	if (keys_cnt > 9) {
		key = keys[8];
		UT_ASSERTeq(kv.get(key, &value), status::OK);
