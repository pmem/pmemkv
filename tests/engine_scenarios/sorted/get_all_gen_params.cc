...
static void GetAllTest(std::string engine, pmem::kv::config &&config)
{
	...
	verify_get_all_c(kv, 16, expected);
	...
	expected = kv_list{{"AA", "8"},	   {"AB", "2"},	 {"AC", "3"},  {"B", "4"},
			   ...
			   {"yyy", "记!"}, {"记!", "RR"}};
	verify_get_all_c(kv, 14, expected);
			   ...
			   const size_t items, const size_t max_key_len)
{
	...
	for (size_t i = 0; i < items; i++) {
		...
		verify_get_all(kv, i + 1, expected);
	}
			   ...
			   const size_t max_key_len)
{
	...
	/* testing C-like API */
	verify_get_all_c(kv, 3, expected);
	...
	for (size_t i = 0; i < keys_cnt; i++) {
		...
		if (i % 5 == 0) {
			verify_get_all(kv, i + 4, expected);
		}
	}
				  ...
				  const size_t max_key_len)
{
	...
	for (size_t i = keys_cnt; i > 0; i--) {
		...
		verify_get_all(kv, keys_cnt - i + 1, expected);
	}
	...
	if (keys_cnt > 20) {
		...
		expected.erase(std::remove(expected.begin(), expected.end(),
			       ...
			       expected.end());
		keys_cnt--;
		verify_get_all_c(kv, keys_cnt, expected);
	}
	if (keys_cnt > 9) {
		key = keys[8];
		UT_ASSERTeq(kv.get(key, &value), status::OK);
