		   ...
		   const size_t cnt)
{
	...
	for (size_t k = 0; k < cnt; k++) {
		...
		std::string generated_key = key_from_number(k, "", postfix);
		keys.push_back(std::move(generated_key));
	}
}
					     ...
					     const size_t max_key_len, pmem::kv::db &kv)
{
	...
	for (size_t i = 0; i < initial_count; i++) {
		std::string key = key_from_number(i, "in_");
		std::string val = key_from_number(i, "in_", "!");
