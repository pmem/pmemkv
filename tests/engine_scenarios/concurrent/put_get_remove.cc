...
static void MultithreadedTestRemoveDataAside(pmem::kv::db &kv)
{
	...
	parallel_exec(threads_number, [&](size_t thread_id) {
		if (thread_id % 2 == 1) {
			auto s = kv.get(uint64_to_key(keys[thread_id]), [&](string_view value) {
	 			UT_ASSERT(value.compare(uint64_to_key(keys[thread_id])) == 0);
	 		});
	 		UT_ASSERTeq(s, status::OK);

			 s = kv.get(uint64_to_key(keys[thread_id - 1]), [&](string_view value) {
	 			UT_ASSERT(value.compare(uint64_to_key(keys[thread_id - 1])) == 0);
	 		});
			...
			UT_ASSERT(s == status::OK || s ==  status::NOT_FOUND);
	 		...
	 		UT_ASSERTeq(kv.remove(uint64_to_key(keys[thread_id])), status::OK);
	 	}
	});
}
...
static void MultithreadedTestRemoveDataAside2(pmem::kv::db &kv)
{
	...
	for (int i = 0; i < size; i += 2)
		UT_ASSERT(kv.put(uint64_to_key(keys[i]), uint64_to_key(keys[i])) == status::OK);
	...
	parallel_exec(threads_number, [&](size_t thread_id) {
		if (thread_id % 2 == 0) {
			UT_ASSERTeq(kv.remove(uint64_to_key(keys[thread_id])), status::OK);
	 		...
	 		UT_ASSERT(kv.put(uint64_to_key(keys[thread_id]), uint64_to_key(keys[thread_id])) == status::OK);
	 	}
	});
}
