...
static void MoveDBTest(pmem::kv::db &kv)
{
	...
	db kv_assign_new2 = std::move(kv_assign_new);

	ASSERT_STATUS(kv_assign_new2.put("key3", "value3"), status::OK);

	ASSERT_STATUS(kv_assign_new2.get("key2", &value), status::OK);
	UT_ASSERT(value == "value2");
	ASSERT_STATUS(kv_assign_new2.get("key3", &value), status::OK);
	UT_ASSERT(value == "value3");

	ASSERT_STATUS(kv_assign_new2.remove("key2"), status::OK);
	ASSERT_STATUS(kv_assign_new2.remove("key3"), status::OK);

	kv_assign_new2.close();
}
