...
static void SimpleTest(kv_to_test &kv)
{
...
static void EmptyKeyTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
	...
	ASSERT_VALUE(value1, "empty");
	ASSERT_STATUS(kv.exists(" "), status::OK);
	ASSERT_STATUS(kv.get(" ", &value2), status::OK);
	ASSERT_VALUE(value2, "1-space");
	ASSERT_STATUS(kv.exists("\t\t"), status::OK);
	ASSERT_STATUS(kv.get("\t\t", &value3), status::OK);
	ASSERT_VALUE(value3, "two-tab");
}

static void EmptyValueTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
...
static void EmptyKeyAndValueTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
...
static void GetClearExternalValueTest(kv_to_test &kv)
{
	ASSERT_STATUS(kv.put("key1", "cool"), status::OK);
	std::string value = "super";
	ASSERT_STATUS(kv.get("key1", &value), status::OK);
	ASSERT_VALUE(value, "cool");
...
static void GetHeadlessTest(kv_to_test &kv)
{
...
static void GetMultipleTest(kv_to_test &kv)
{
...
static void GetMultiple2Test(kv_to_test &kv)
{
	...
	ASSERT_VALUE(value1, "value1");
	std::string value2;
	ASSERT_STATUS(kv.get("key2", &value2), status::NOT_FOUND);
	std::string value3;
	ASSERT_STATUS(kv.get("key3", &value3), status::OK);
	ASSERT_VALUE(value3, "VALUE3");
}

static void GetNonexistentTest(kv_to_test &kv)
{
	ASSERT_STATUS(kv.put("key1", "value1"), status::OK);
	ASSERT_STATUS(kv.exists("waldo"), status::NOT_FOUND);
	std::string value;
...
static void PutTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
	...
	ASSERT_VALUE(new_value2, "new_value");
	...
	ASSERT_VALUE(new_value3, "?");
}

static void PutValuesOfDifferentSizesTest(kv_to_test &kv)
{
	std::string value;
	...
	ASSERT_VALUE(value, "123456789ABCDE");
	...
	ASSERT_VALUE(value2, "123456789ABCDEF");
	...
	ASSERT_VALUE(value3, "12345678ABCDEFG");
	...
	ASSERT_VALUE(value5, "123456789ABCDEFGHI");
}

static void RemoveAllTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
...
static void RemoveAndInsertTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
...
static void RemoveExistingTest(kv_to_test &kv)
{
	std::size_t cnt = std::numeric_limits<std::size_t>::max();
	ASSERT_STATUS(kv.count_all(cnt), status::OK);
	UT_ASSERT(cnt == 0);
	...
	ASSERT_VALUE(value, "tmpvalue2");
}

static void RemoveHeadlessTest(kv_to_test &kv)
{
	ASSERT_STATUS(kv.remove("nada"), status::NOT_FOUND);
}

static void RemoveNonexistentTest(kv_to_test &kv)
{
	ASSERT_STATUS(kv.put("key1", "value1"), status::OK);
	ASSERT_STATUS(kv.remove("nada"), status::NOT_FOUND);
	ASSERT_STATUS(kv.exists("key1"), status::OK);
