...
static const char *EXPECTED_ERR_MSG = "[pmemkv_open] Comparator with name: \"valid_cmp\" expected";
...
{
    pmemkv_comparator *cmp = pmemkv_comparator_new(NULL);
    UT_ASSERTne(cmp, NULL);

    int s = pmemkv_comparator_set_function(cmp, compare);
    UT_ASSERTeq(s, PMEMKV_STATUS_OK);

    s = pmemkv_comparator_set_name(cmp, name);
    UT_ASSERTeq(s, PMEMKV_STATUS_OK);
...
static void insert(const char *engine, pmemkv_config *cfg)
{
    ...
    int s = pmemkv_config_put_object(cfg, "comparator", (void*)cmp, NULL);
    UT_ASSERTeq(s, PMEMKV_STATUS_OK);
